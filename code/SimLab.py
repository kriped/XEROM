...
def initialise_global_variables(filename:str):
   ...
   #no_axial_nodes = int(re.findall(r'(?<=KD\s\s)(\d\d)', ''.join(lines))[0])
    ...
    tf = 75 #hours
    dt = 0.5 #hours
    n_timesteps = int(((tf+dt)-t0)/dt)
    list_length = int(no_radial_nodes*no_radial_nodes*no_axial_nodes)
    time_array = np.arange(t0,tf+dt,dt)
    mem_case = int(0)
    output_FLX1 = [[[[None]*no_radial_nodes]*no_radial_nodes]*no_axial_nodes]*n_timesteps
    output_FLX2 = [[[[None]*no_radial_nodes]*no_radial_nodes]*no_axial_nodes]*n_timesteps
    output_Iodine = [[[[None]*no_radial_nodes]*no_radial_nodes]*no_axial_nodes]*n_timesteps
    output_Xenon = [[[[None]*no_radial_nodes]*no_radial_nodes]*no_axial_nodes]*n_timesteps
...
def case_line_identifier(line:str):
    ...
    """
    identifier = re.search(r'Case\s+\d+',line)
    if identifier:
        return True
    else:
        return False
...
class Nodal_Values:
    ...
    def __init__(self,variable_match:str):
        ...
        try:
            print(f"current line number is {line_number}")
        except:
            pass
        print(f"The instance {self.name} has been created")
    def __del__(self):
        print(f"current line number is {line_number}")
        print(f"The instance {self.name} is being destroyed")
...
class XY_Nodal_Values(Nodal_Values):
    ...
    def write_data_to_dict(self):
        mydict[f"{current_case.case_name}_{self.variable_match}"] = self.data
...
class XZ_Nodal_Values(Nodal_Values):
    ...
    def __init__(self, variable_match: str):
        ...
        self.output_array = np.zeros((no_radial_nodes,no_radial_nodes,no_axial_nodes,n_timesteps))
    def get_y_index(self,line:str):
        try:
    ...
    def write_data_to_dict(self):
        """
        When the all the data has been collected for a given variable, writes to either a stationary spatial destination or a time dependent destiantion in mydict.
        """
        if current_case_number == 5:
            self.output_array[:, :, :, current_step_number] = self.data
            if current_step_number == 145:
                mydict[f"{current_case.case_name}_output_{self.name}"] = self.output_array
        else:
            mydict[f"{current_case.case_name}_{self.name}"] = self.data
        print(f"{current_case.case_name} - variable {self.name} has been written to the dictionary and is ready for export")
...
if __name__ == "__main__":
    ...
    for case_number in cases:
        globals()[f"case_{case_number}"] = Case(case_number)
        case_instance = globals()[f"case_{case_number}"]

        #print(f"Initialising {case_instance.case_name}")
        
        for variable in [a for a in dir(case_instance) if not a.startswith('__')]:
            attr_value = getattr(case_instance,variable)
            
            if isinstance(attr_value,(XY_Nodal_Values, XZ_Nodal_Values)):
                mydict[f"Case_{case_number}_{variable}"] = attr_value.data
            else:
                mydict[f"Case_{case_number}_{variable}"] = attr_value
        #print(f"{case_instance.case_name} initialised")
    ...
    for line in lines:
        ...
        if line.strip() == "":
            continue
        if (search_data and not (bool(re.match(r'^\s*\d{1,2}(?!\w)',line)) or bool(re.match(r'^\s*\( *(\d+)\)',line)))): 
                    continue
        if (search_case and case_line_identifier(line)): #When search_case is true the search for the case number is active
            current_case_number = get_case_number(line)
            try:
                current_case =  globals()[f"case_{current_case_number}"]
            except KeyError:
                search_case = False
            if current_case_number == 5:
            ...
            continue
        elif (search_case == True):
            continue     
        if (re.search(r"PRI.MAC\s+-\s+Cross section",line) and search_variable == True): #This search should get the cross sections in XY plane
            ...
            continue
        elif (re.search(r"PRI.STA \w+\s+-\s+NODAL 3D",line) and search_variable == True):
            for variable_match in current_case.get_XZ_variable_match_list(): # Figure out which variable is being loaded
                if variable_line_identifier(line,variable_match):
                    ...
                    break # End loop through variables when variable is found
            continue
        elif (search_variable == True and line.strip() != ""): #This condition might be a bit hard and activate case search too soon in some cases
            Case_search_activate()
            continue

        if (search_indices and bool(re.match(r'^\s+(\d+)(?:\s+(\d+)){16}\s*$',line))): # This line is requires that there are 34 nodes in the system.
            current_x_indices = current_variable.get_x_indices(line)
            current_x_indices = [item - 1 for item in current_x_indices]
            Data_search_activate()
            continue
        if search_data: 
            if isinstance(current_variable,XY_Nodal_Values):
                ...
                if current_x_indices[0] == 0:
                    ...
                    if current_y_index == no_radial_nodes: # This is the last line of data for this z index
                        ...
                        if current_z_index == no_axial_nodes-1:
                            current_variable.write_data_to_dict()
            ...
            if isinstance(current_variable,XZ_Nodal_Values):
                if (search_y_index and re.search(r'\( *(\d+)\)',line)):
                    current_y_index = current_variable.get_y_index(line)
                ...
                if current_x_indices[0] == 0:
                    if current_y_index % 2 == 1:
                        current_variable.data[current_x_indices[-current_length:],current_y_index-1,current_z_index] = [value / current_scaling for value in current_line_data]
                        ...
                        if current_z_index == 0: # This the last line for this half of the symmetry in this z index
                            Indices_search_activate()
                            continue
                else:
                    if current_y_index % 2 == 1:
                        current_variable.data[current_x_indices[0:current_length],current_y_index-1,current_z_index] =  [value / current_scaling for value in current_line_data]
        ...
        if search_summary:
            try:
                if mem_case != current_case_number:
                    mem_case = current_case_number
    ...
    sio.savemat(file_name+".mat",mydict)
