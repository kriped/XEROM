function data = prepareAOComparison_test(data,opts)
    ...
    r_detrendResults = detrend(tr_seg,R(opts.skip_mscnpp+1:end),opts.AOfit);
    r_seg = r_detrendResults.detrended; % Store the detrended segment for further processing
    x_detrendResults = detrend(tx_seg,X(opts.skip_xerom+1:end),opts.AOfit);
