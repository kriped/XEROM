...
if __name__ == "__main__":
    ...
    for line in lines:
        ...
        if (re.search(r"PRI.MAC\s+-\s+Cross section",line) and search_variable == True): #This search should get the cross sections in XY plane
            ...
            continue
        if (re.search(r"PRI.STA \w+\s+-\s+NODAL 3D",line) and search_variable == True):
                    ...
                    if current_x_indices[0] == 0:
                        ...
                        if current_y_index % 2 == 1:
                            ...
                            if current_z_index == 0: # This is the last line of data for this z index
                                if current_y_index == no_radial_nodes: # This is the last line of data for the variable
                                    ...
                                    if current_step_number is not None:
                                        current_variable.data_array[current_step_number*list_length:(current_step_number+1)*list_length] =  current_variable.data.ravel()
                                    Case_search_activate()
                            ...
                            if current_z_index == 0: # This is the last line of data for this z index
                                if current_y_index == no_radial_nodes: # This is the last line of data for the variable
                                    if current_step_number is not None:
                                        if current_step_number == n_timesteps:
                                            current_variable.data_array[current_step_number*list_length:(current_step_number+1)*list_length] =  current_variable.data.ravel()
                                            mydict[current_case.case_name+"_"+current_variable.name+"data_array"] = current_variable.data_array  
                                        else:
                                            continue                               
                                    else:
                                        mydict[current_case.case_name+"_"+current_variable.name] = current_variable.data
